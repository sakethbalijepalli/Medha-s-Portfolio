...
export async function signInWithGoogle(): Promise<void> {
  const { error } = await supabase.auth.signInWithOAuth({
    provider: "google",
    options: {
      // Portfolio, unlike standalone DanceTracker, serves multiple routes — without this,
      // Supabase sends the callback to its configured Site URL instead of back to /tracker.
