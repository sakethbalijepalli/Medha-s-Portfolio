...
// The public portfolio's index.html never loads these — DanceTracker's "spotlight"
// theme (Fraunces + Instrument Sans) is fetched only when this route actually mounts,
// so marketing pages don't pay for fonts they never use.
function useTrackerFonts() {
  useEffect(() => {
    if (document.getElementById("tracker-fonts")) return;
    ...
    stylesheet.id = "tracker-fonts";
    stylesheet.rel = "stylesheet";
    stylesheet.href =
      "https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,400;0,9..144,500;0,9..144,600;1,9..144,500&family=Instrument+Sans:ital,wght@0,400;0,500;0,600;0,700;1,500&display=swap";
  ...
  useEffect(() => {
    const handleSession = (newSession: Session | null) => {
      if (newSession && !isAllowedEmail(newSession.user.email)) {
        // Not Medha's account — drop the session immediately, before it ever
        // reaches state that would render opportunity data or trigger a calendar
        // token save for a stranger's sign-in.
      ...
      <div className="tracker-root">
        <div className="signin-screen">
          <div className="signin-card">
            ...
            {unauthorized ? (
              <p role="alert">This Google account isn't authorized for DanceTracker.</p>
            ) : (
